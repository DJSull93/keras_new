...
x_train = pad_sequences(x_train, padding='pre', maxlen=11) # 'pre' <> 'post'
x_pred = pad_sequences(x_pred, padding='pre', maxlen=11) # 'pre' <> 'post'
...
from tensorflow.keras.layers import Dense, Embedding, LSTM, GRU, Dropout, Conv1D, GlobalAveragePooling1D, MaxPool1D, Bidirectional

model = Sequential()
model.add(Embedding(input_dim=101082, output_dim=16,
                     input_length=11))
...
'''
time :  15.76770830154419
loss :  0.7676664590835571
acc :  0.7514965534210205
'''