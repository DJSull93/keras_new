...
앞으로 나올 대부분의 모델은 그렇지 못함

레이어간의 연산 수가 많아지면서 웨이트가 급격히 커지기 때문

# 모델의 계산 횟수
것처럼 model.summary()를 통해 모델이 제대로 구성이
되었는지와, parameter가 몇개인지 확인 할 수 있고,

여기서 모델의 계산 횟수는 전체 parameter 개수와 동일함
이때, 파라미터의 개수는 노드의 수에 영향을 받고, 
깊어질수록, 노드가 많아질수록 기하급수적으로 연산이 늘어나며, 
이에 따라 웨이트는 너무 큰 수가 됨
...
따라서 연산 효율이 상승하고, 정확도의 개선을 도모할 수 있음

활성함수 또한 다양하게 존재하며, 기본값은 linear이며 최종 아웃풋은 
기본값을 사용해야만 함

그외, 입력 및 중간 레이어에서는 relu라는 강력한 함수를 사용
